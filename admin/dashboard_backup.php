...
$stmt = $pdo->query("SELECT 
    'maintenance' as type, issue_type as description, created_at as created_at, priority
    FROM maintenance_requests 
...
$stmt = $pdo->query("SELECT 
    'offense' as type, CONCAT(offense_type, ' - ', description) as description, created_at as created_at, severity
    FROM offenses 
    WHERE status = 'pending' 
    ORDER BY created_at DESC 
    LIMIT 5");
...
$stmt = $pdo->query("SELECT 
    'complaint' as type, subject as description, created_at as created_at, status
    FROM complaints 
...
$stmt = $pdo->query("SELECT 
    ...
    SUM(CASE WHEN r.occupied >= r.capacity THEN 1 ELSE 0 END) as occupied_rooms,
    SUM(CASE WHEN r.occupied = 0 AND r.status = 'available' THEN 1 ELSE 0 END) as available_rooms
    FROM buildings b
...
const offenseData = {
    ...
    severe: <?php echo isset(array_column($offense_by_severity, 'count', 'severity')['severe']) ? array_column($offense_by_severity, 'count', 'severity')['severe'] : 0; ?>
};
...
new Chart(offenseCtx, {
    ...
    data: {
        ...
        datasets: [{
            data: [offenseData.minor, offenseData.major, offenseData.severe],
            backgroundColor: [
                '#28a745',
                '#ffc107',
                '#dc3545'
